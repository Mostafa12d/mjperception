"""The RLS baseline wrapped in the ``OnlineAdaptor`` interface, calling
``dyn.rls_init`` / ``rls_step`` unmodified.

Three choices keep the comparison fair, all favouring RLS: a 5-parameter
spring-aware regressor ``tau = I*thdd + mu*sign(thd) + b*thd + k*th + c`` (the
baseline's own 3-parameter form is misspecified for the 70% of doors with a
closer, and is available as ``n_params=3``); velocity gating, since at rest the
equation of motion is an inequality; and sub-stepped integration at MuJoCo's own
0.002 s, so RLS is not charged for discretisation error.

The remaining asymmetry: acceleration is not observable, so the regressor
finite-differences the velocities, which is noisier than MuJoCo's exact qacc.
"""
...
# priors matching the baseline's run_door_adaptive_impedance
DEFAULT_I = 5.0
DEFAULT_MU = 3.0
DEFAULT_B = 0.2
I_MIN = 0.1  # the baseline clamps I_hat the same way


class RLSAdaptor(OnlineAdaptor):
    """Recursive least squares on interpretable hinge parameters.

    ``n_params`` is 5 for the spring-aware regressor or 3 for the baseline's own;
    updates are skipped below ``vel_thresh``.
    """
    ...
    def latent(self) -> np.ndarray:
        """The belief, for logging symmetry only: these are physical parameters,
        not comparable to ``z`` and never projected into the embedding PCA."""
        return self.params
    ...
    def _unpack(self) -> tuple[float, float, float, float, float]:
        th = self._rls.theta
        I = max(float(th[0]), I_MIN)  # guard a non-physical early estimate
    ...
    def predict(self, state: ArrayLike, action: ArrayLike) -> np.ndarray:
        ...
        for _ in range(self.n_substeps):
            net = tau - b * thd - k * th - c   # all but inertia and Coulomb friction
            if abs(thd) <= 1e-9:
            ...
            thd_new = thd + acc * h
            # friction may decelerate but not reverse within a substep, or the
            # model chatters at rest
